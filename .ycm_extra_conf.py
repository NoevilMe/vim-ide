...
# for clangd, https://github.com/ycm-core/ycmd/#c-family-settings
def Settings(**kwargs):
    '''
    The Settings function is called by the libclang and clangd-based completers
    to get the compiler flags to use when compiling the current file. The abso-
    lute path of this file is accessible under the filename key of the kwargs
    dictionary.

    kwargs contains language, filename, client_data

    The return value expected by both completers is a dictionary containing the
    following items:
    - flag: (mandatory for libclang, optional for clangd) a list of compiler
    flags.
    - include_paths_relative_to_dir (optional)
    - o_cache: (optional) Defaults to True. If unsure, the default is almost a-
    lways correct.
    '''
    ...
    # return { 'project_directory': 'src/'  } # The path may be absolute as
    # well.
    return {}
